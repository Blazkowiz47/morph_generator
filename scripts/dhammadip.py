...
def copy_aligned_perturbed_images_locally():
    for dataset_dir in RDIR.glob("*"):
        if not dataset_dir.is_dir():
            continue
        dataset = dataset_dir.name
        for experiment_dir in dataset_dir.glob("*"):
            if not (RDIR / experiment_dir).is_dir():
                continue
            if "original" == experiment_dir.name:
                continue
            experiment = experiment_dir.name

            aligned_dir = experiment_dir / "aligned"
            for ssplit in ["train", "test"]:
                split_dir = aligned_dir / ssplit
                if not split_dir.is_dir():
                    continue

                src_dir = str(split_dir)
                out_dir = str(
                    Path("/home/ubuntu/datasets/dhammadip")
                    / dataset
                    / experiment
                    / "aligned"
                    / ssplit
                ...
                os.makedirs(out_dir, exist_ok=True)
                for image in tqdm(Path(src_dir).glob("*"), desc=f"Copying {src_dir}"):
                    if image.suffix.lower() not in [".jpg", ".jpeg", ".png"]:
                        continue
                    o_path = image.as_posix().replace(src_dir, out_dir)
                    Image.open(image).save(o_path)
...
if __name__ == "__main__":
    ...
    # face_detect_images()
    copy_aligned_perturbed_images_locally()
